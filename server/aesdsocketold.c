...
static void handle_signal(int sig)
{
    (void)sig;
    g_run = 0;
    if (g_listen_fd >= 0) shutdown(g_listen_fd, SHUT_RDWR); // unblock accept()
}
...
int main(int argc, char *argv[])
{
    ...
    // start fresh each run (your choice—matches earlier behavior)
    unlink(DATAFILE);
    ...
    /* -------- added: daemonize after binding, before listen -------- */
    if (run_daemon) {
        if (daemonize() < 0) {
    ...
    while (g_run) {
        ...
        if (connfd < 0) {
            if (!g_run) break;              // interrupted by signal
        ...
        if (addrptr) inet_ntop(their_addr.ss_family, addrptr, ipstr, sizeof ipstr);
        syslog(LOG_INFO, "Accepted connection from %s", ipstr);

        /* ---------- (e) recv newline-delimited packets, append ---------- */
        ...
        /* flush leftover tail if client closed without '\n' 
        if (pkt && pkt_len > 0) {
            if (write(data_fd, pkt, pkt_len) < 0) {
                syslog(LOG_ERR, "write(%s): %s", DATAFILE, strerror(errno));
            } else {
                fsync(data_fd);
                (void)send_entire_file(connfd);
            }
            pkt_len = 0;
        }*/
